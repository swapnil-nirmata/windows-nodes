...
function InstallPauseImage() {
    ...
    if (!$infraPodImage)
    {
        ...
        Invoke-WebRequest -Uri "https://github.com/$GithubSDNRepository/raw/$GithubSDNBranch/Kubernetes/windows/Dockerfile" -OutFile  $ImagePath\Dockerfile -Proxy $webproxy
...
function Install-7Zip() {
    ...
    if (-not (Get-Command Expand-7Zip -ErrorAction Ignore)) {
        Install-Package -Scope CurrentUser -Force 7Zip4PowerShell -Verbose -proxy $webproxy
        if(-not $?) {
...
InstallPauseImage -ImagePath $Global:BaseDir -GithubSDNRepository $GithubSDNRepository -GithubSDNBranch $GithubSDNBranch -webproxy $webproxy
