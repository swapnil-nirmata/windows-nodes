Param(
    ...
    [parameter(Mandatory = $false)] $GithubSDNRepository = 'Microsoft/SDN',
    [parameter(Mandatory = $false)] [switch] $Reset=$false,
    [parameter(Mandatory = $false)] $GithubSDNBranch = 'master',
    [parameter(Mandatory = $false)] $DockerVersion = "20.10.9",
#    [parameter(Mandatory = $false)] $webproxy = "http://nibr-proxy.global.nibr.novartis.net:2011",
    [parameter(Mandatory = $false)] $Url=  "https://dl.k8s.io/v${Release}/kubernetes-node-windows-amd64.tar.gz"    
)
...
#        $webproxy = "http://nibr-proxy.global.nibr.novartis.net:2011" 
    ...
    if (!$infraPodImage)
    {
        ...
        Invoke-WebRequest -Uri "https://github.com/$GithubSDNRepository/raw/$GithubSDNBranch/Kubernetes/windows/Dockerfile" -OutFile  $ImagePath\Dockerfile
...
function InstallKubernetesBinaries() {
    ...
    Param(
        [parameter(Mandatory = $true)] $Release,
            $DestinationPath,
            $BinaryUrl =  "https://dl.k8s.io/v${Release}/kubernetes-node-windows-amd64.tar.gz"
#            $webProxy = "http://nibr-proxy.global.nibr.novartis.net:2011"
        ) 
...
function Get-KubeFlannelPath() {
    #return "c:\k\etc\kube-flannel"
    return [io.Path]::Combine($Global:BaseDir, "flannel");
}

function DownloadFlannelBinaries() {
    param(
        [Parameter(Mandatory = $false, Position = 0)] [string] $Release = "0.13.0",
        [parameter(Mandatory = $false)] [string] $DestinationPath = "c:\k\flannel"
#        [parameter(Mandatory = $false)] $webProxy = "http://nibr-proxy.global.nibr.novartis.net:2011"
    )
    ...
    Write-Host "$DestinationPath\flanneld.exe"
    Invoke-WebRequest -Uri "https://github.com/coreos/flannel/releases/download/v${Release}/flanneld.exe" -OutFile $DestinationPath\flanneld.exe
...
#        [parameter(Mandatory = $false)] $webProxy = "http://nibr-proxy.global.nibr.novartis.net:2011"
    ...
    Invoke-WebRequest -Uri "https://github.com/$Global:GithubSDNRepository/raw/$Global:GithubSDNBranch/Kubernetes/flannel/$NetworkMode/cni/config/cni.conf" -OutFile $CniPath\config\cni.conf
}
...
function Get-KubeletPath() {
    #return "c:\k\etc\kube-flannel"
    return [io.Path]::Combine($Global:BaseDir, "kubelet");
}
...
function InstallKubelet()
{
    ...
    (
        ...
        [parameter(Mandatory = $true)] $KubeletPath,
...
function CreateExternalNetwork
{
    ...
    Write-Output "Open firewall for Overlay traffic"
    ...
    {
        Write-Output "Create a Overlay network to trigger a vSwitch creation"
...
function Get-KubeProxyPath() {   
    return [io.Path]::Combine($Global:BaseDir, "KubeProxy");
}
...
Write-Output "Install K8 binaries"
InstallKubernetesBinaries -BinaryUrl $Kuber -DestinationPath $Global:BaseDir -Release $Global:Release

# Download Flannel
Write-Output "Download Flannel bins"
$flannelDir = Get-KubeFlannelPath
New-Directory $flannelDir
DownloadFlannelBinaries -DestinationPath $Global:BaseDir

# Install CNI binaries
Write-Output "Download CNI bins"
DownloadCniBinaries -NetworkMode $Global:NetworkMode -CniPath (GetCniPath)

# Install Flannel
Write-Output "Install Flannel"
InstallFlannelD -Destination $Global:BaseDir -InterfaceIpAddress $Global:ManagementIp

# CreateDirectory (GetCniConfig)
Write-Output "Update CNI Config"
...
Write-Output "Update Network Config"
Update-NetConfig -NetConfig (GetFlannelNetConf) `
-ClusterCIDR (GetClusterCidr) `
-NetworkName $Global:NetworkName -NetworkMode $Global:NetworkMode

# Install Kubelet
Write-Output "Installing Kubelet" 
$kubeletDir = Get-KubeletPath
New-Directory $kubeletDir
InstallKubelet -KubeConfig (GetKubeConfig) -CniDir (GetCniPath) `
-CniConf $(GetCniConfigPath) -KubeDnsServiceIp (GetKubeDnsServiceIp) `
-NodeIp $Global:ManagementIp -KubeletPath $kubeletDir
Write-Output "Starting Kubelet"
StartKubelet
Write-Output "Waiting for Kubelet"
WaitForNodeRegistration -TimeoutSeconds 10

# Install CNI & Flannel
Write-Output "Create External Network, system will be restarted as a part of this process"
...
Write-Output "Creating KubeProxy Directories"
$kubeProxyDir = Get-KubeProxyPath
New-Directory $kubeProxyDir
$sourceVip = GetSourceVip -NetworkName $Global:NetworkName
Write-Host "Installing KubeProxy"
